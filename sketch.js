var settings = {
  shipSpeed: 0.2,
  shipRotationSpeed: Math.PI / 30.5,
  bulletLife: 60,
  bulletSpeed: 8,

  largeAsteroidSpeed: 1,
...
class Game {
  constructor() {
    ...
    settings.bulletLife =
      min(window.innerHeight, window.innerWidth) / 2 / settings.bulletSpeed;
  ...
  initializeAstroids() {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      var x =
        window.innerWidth / 2 +
        (xBuffer + (window.innerWidth / 2 - xBuffer) * random()) * randomSign();
      var y =
        window.innerHeight / 2 +
        (yButter + (window.innerHeight / 2 - yButter) * random()) *
          randomSign();
      ...
      this.asteroids.push(
        new LargeAsteroid(
          new Point(x, y),
          new Velocity().set(
            settings.largeAsteroidSpeed * this.level,
            2 * PI * random()
          )
        )
      );
  ...
  wrapAroundEdge(obj) {
    if (obj.point.x > window.innerWidth)
      obj.point.x = obj.point.x - window.innerWidth;
    if (obj.point.x < 0) obj.point.x = obj.point.x + window.innerWidth;
    if (obj.point.y > window.innerHeight)
      obj.point.y = obj.point.y - window.innerHeight;
  ...
  draw() {
    ...
    for (var i in this.asteroids) {
      ...
      if (x < this.asteroids[i].radius)
        this.asteroids[i].drawAtPoint(new Point(x + window.innerWidth, y));
      if (x > window.innerWidth - this.asteroids[i].radius)
        this.asteroids[i].drawAtPoint(new Point(x - window.innerWidth, y));
      if (y < this.asteroids[i].radius)
        this.asteroids[i].drawAtPoint(new Point(x, y + window.innerHeight));
      if (y > window.innerHeight - this.asteroids[i].radius)
        this.asteroids[i].drawAtPoint(new Point(x, y - window.innerHeight));
    }
    ...
    var formattedScore = ("000000" + this.score).slice(-6);
  ...
  rotateRight() {
    this.ship.rotate(settings.shipRotationSpeed);
  }
  ...
  thrust() {
    if (this.ship.alive) {
      var thrust = new Velocity().set(settings.shipSpeed, this.ship.rotation);
  ...
  fire() {
    if (this.ship.alive) {
      var bulletVelocity = new Velocity().set(
      ...
      this.bullets.push(
        new Bullet(bulletPoint, bulletVelocity, this.ship.velocity.getMagnitude())
      );
    }
  }
  ...
  handleCollisions() {
    for (var i in this.asteroids) {
      for (var j in this.bullets) {
        if (
          this.getClosestDistance(this.asteroids[i], this.bullets[j]) <
            this.asteroids[i].radius &&
          this.bullets[j].alive &&
          this.asteroids[i].alive
        ) {
          this.breakAsteroid(this.asteroids[i]);
          this.asteroids[i].alive = false;
          this.bullets[j].alive = false;
          this.score += round(this.bullets[j].sourceVelocity);
          console.log(round(this.bullets[j].sourceVelocity));
        }
      }

      if (
        this.getClosestDistance(this.asteroids[i], this.ship) <
        this.asteroids[i].radius / 2 + this.ship.radius
      ) {
        this.ship.alive = false;
  ...
  breakAsteroid(asteroid) {
    switch (asteroid.radius) {
      case new LargeAsteroid().radius:
        this.asteroids.push(
          this.generateAsteroid(
            new MediumAsteroid(),
            asteroid,
            0 + randomPM() * this.level,
            1 + randomPM() * this.level
          )
        );
        this.asteroids.push(
          this.generateAsteroid(
        ...
        this.asteroids.push(
          this.generateAsteroid(
            new SmallAsteroid(),
            asteroid,
            2 + randomPM() * this.level,
            0 + randomPM() * this.level
          )
        );
        this.score += settings.largeAsteroidPoints;
        break;
      ...
      case new MediumAsteroid().radius:
        ...
        this.score += settings.mediumAsteroidPoints;
        break;

      case new SmallAsteroid().radius:
        this.score += settings.smallAsteroidPoints;
        break;
    }
  }
  ...
  getClosestDistance(obj1, obj2) {
    ...
    for (var i = 0.0; i <= dMax; i++) {
      var point1 = new Point(
        obj1.point.x + (obj1.velocity.dx * i) / dMax,
        obj1.point.y + (obj1.velocity.dy * i) / dMax
      );
      var point2 = new Point(
        obj2.point.x + (obj2.velocity.dx * i) / dMax,
...
class Bullet extends FlyingObject {
  constructor(
    ...
    this.radius = 1;
    this.rotation = -PI / 2;
    this.remainingFrames = settings.bulletLife;
...
function generateAsteroidShape(radius) {
  ...
  for (var i = 0.0; i < 2 * PI; i += (2 * PI) / sides) {
    points.push(
      new Point(
        radius * cos(i) + rFactor * randomPM(),
        radius * sin(i) + rFactor * randomPM()
      )
    );
  }
  return points;
